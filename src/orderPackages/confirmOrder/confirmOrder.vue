<template>
  <scroll-view scroll-y class="viewport">
    <!-- 收货地址 -->
    <navigator v-if="defaultAddress" class="shipment" hover-class="none" url="/userPackages/address/address?from=order">
    ...
    <navigator v-else class="shipment" hover-class="none" url="/userPackages/address/address?from=order">
    ...
    <!-- 商品信息 -->
    <view class="goods">
      <navigator v-for="item in orderList?.goods" :key="item.skuId" :url="`/pages/goods/goods?id=${item.id}`" class="item"
        hover-class="none">
        ...
        <view class="meta">
          ...
          <view class="prices">
            ...
            <view class="price symbol">{{ item.payPrice }}</view>
          </view>
    ...
    <view class="related">
      ...
      <view class="item">
        ...
        <input class="input" :cursor-spacing="30" placeholder="选填，请先和商家协商一致" v-model="buyerMessage" />
      </view>
    </view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: !defaultAddress?.id }" @tap="orderSubmit"> 提交订单 </view>
  </view>
</template>

<script setup lang="ts">
import { getPreOrderList, getPreOrderListNow, postOrderList } from '@/api/order';
import type { OrderPreResult } from '@/api/orderType';
...
const getPreOrderData = async () => {
  if (query.skuId && query.count) {
    const res = await getPreOrderListNow({ skuId: query.skuId, count: query.count })
    orderList.value = res.result
  } else {
    const res = await getPreOrderList()
    orderList.value = res.result
  }
}
...
const orderSubmit = async () => {
  if (!defaultAddress.value?.id) return uni.showToast({ title: '请选择你的收货地址', icon: 'none' })
  ...
  uni.redirectTo({ url: `/orderPackages/detail/detail?id=${res.result.id}` })
}
