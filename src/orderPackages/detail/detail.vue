  ...
  <scroll-view scroll-y class="viewport" id="scroller" @scrolltolower="onScrolltolower">
    <template v-if="order">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-if="order?.orderState == OrderState.DaiFuKuan">
          ...
          <view class="tips">
            ...
            <uni-countdown :second="order.countdown" @timeup="timeUp" color="#fff" :show-day="false" :show-colon="false"
              splitor-color="#fff" />
          </view>
          <view class="button" @tap="orderPay">去支付</view>
        </template>
        <!-- 其他订单状态:展示再次购买按钮 todo:点击再次购买进入订单填写-->
        <template v-else>
          <!-- 订单状态文字 -->
          ...
          <view class="button-group">
            ...
            <view v-if="order.orderState == OrderState.DaiFaHuo" @tap="orderSend" class="button"> 模拟发货 </view>
            <view v-if="order.orderState == OrderState.DaiShouHuo" @tap="orderConfirm" class="button"> 确认收货 </view>
          </view>
        </template>
      </view>
      <!-- 配送状态 -->
      ...
      <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
        <!-- 待付款状态:展示支付按钮 -->
        <template v-if="order.orderState == OrderState.DaiFuKuan">
          <view class="button primary" @tap="orderPay"> 去支付 </view>
        ...
        <template v-else>
          ...
          <!-- 待收货状态: 展示确认收货 -->
          <view class="button primary" v-if="order.orderState == OrderState.DaiShouHuo" @tap="orderConfirm"> 确认收货 </view>
          <!-- 待评价状态: 展示去评价 -->
          <view class="button" v-if="order.orderState == OrderState.DaiPingJia"> 去评价 </view>
          <!-- 待评价/已完成/已取消 状态: 展示删除订单 -->
          <view class="button delete" v-if="order.orderState >= OrderState.DaiPingJia" @tap="orderDelete"> 删除订单 </view>
        </template>
      </view>
    </template>
    <template v-else>
...
import { getOrderConsignmentById, getOrderListById, putOrderReceiptById, getOrderLogisticsById, deleteOrderById } from '@/api/order';
import type { LogisticItem, OrderResult } from '@/api/orderType';
import { OrderState, orderStateList } from '@/api/constant';
...
const orderPay = async () => {
  await getWxPay({ orderId: query.id })
  uni.redirectTo({ url: `/orderPackages/payment/payment?id=${query.id}` })
}
// 模拟发货按钮
const orderSend = async () => {
  await getOrderConsignmentById(query.id)
...
const orderConfirm = () => {
  uni.showModal({
    ...
    success: async ({ confirm, cancel }) => {
      if (confirm) {
        const res = await putOrderReceiptById(query.id)
        order.value = res.result // 更新订单状态
      }
    }
  })
}
...
const orderDelete = () => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        await deleteOrderById({ ids: [query.id] })
        uni.redirectTo({ url: '/orderPackages/orderList/orderList' })
      }
    },
  })
}
...
const getOrderLogisticsDataById = async () => {
  const res = await getOrderLogisticsById(query.id)
...
onReady(() => {
  ...
  pageInstance.animate('.navbar',
    ...
    timeRange: 1000, // 时间长度
    startScrollOffset: 50, // 开始滚动偏移量
    endScrollOffset: 150, // 停止滚动偏移量
  },
  )
  // 动画效果,导航栏标题
