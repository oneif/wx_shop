    ...
    <scroll-view scroll-y class="orders">
        <view class="card" v-for="item in orderList" :key="item.id">
            ...
            <navigator v-for="sku in item.skus" :key="sku.id" class="goods"
                :url="`/orderPackages/detail/detail?id=${item.id}`" hover-class="none">
                <view class="cover">
...
import type { OrderItem, OrderListParams } from '../../api/orderType';
import { orderStateList, OrderState } from '../../api/constant'
...
// 支付按钮事件
const orderPay = async (id: string) => {
    await getWxPay({ orderId: id })
    uni.showToast({ title: '支付成功' })
    const payOrder = orderList.value.find(item => item.id === id)
    payOrder!.orderState = OrderState.DaiFaHuo
}
