<template>
  <view class="viewport">
    ...
    <!-- 推荐选项 -->
    <view class="tabs">
      <text class="text" :class="{ active: index === activeIndex }" v-for="(item, index) in subTypes" :key="item.id"
    ...
    <scroll-view scroll-y class="scroll-view" v-for="(item, index) in subTypes" :key="item.id"
      ...
      <view class="loading-text">{{ item.finish ? '--我也是有底线的--' : '正在加载...' }}</view>
    </scroll-view>
  </view>
</template>

<script setup lang="ts">
import { getHotRecommend } from '@/api/hot'
import type { SubTypeItem } from '@/api/hotType'
...
const getHotRecommendData = async () => {
  const res = await getHotRecommend(currHotMap!.url)
...
const onScrolltolower = async () => {
  ...
  if (currsubTypes.goodsItems.page < currsubTypes.goodsItems.pages) currsubTypes.goodsItems.page++
    ...
    return uni.showToast({ icon: 'none', title: '滑到底啦~' })
  }
  const res = await getHotRecommend(currHotMap!.url, {
